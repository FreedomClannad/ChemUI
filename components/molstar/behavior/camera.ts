...
const DefaultMesoFocusLociBindings = {
	clickCenter: Binding([Trigger(B.Flag.Primary, M.create())], "Camera center", "Click element using ${triggers}"),
	...
	ctor: class extends PluginBehavior.Handler<MesoFocusLociProps> {
		register(): void {
			...
			this.subscribeObservable(this.ctx.behaviors.interaction.key, ({ code, key, modifiers }) => {
				...
				if (Binding.matchKey(b.keyCenterOnly, code, modifiers, key)) this.params.centerOnly = !centerOnly;
			});
		}
	},
